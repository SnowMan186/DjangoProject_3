...
class CoursesAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.lesson1 = Lesson.objects.create(
            owner=self.user1, course=self.course1, title="Урок владельца", video_url="https://youtube.com"
        )
        self.lesson2 = Lesson.objects.create(
            owner=self.user2, course=self.course2, title="Чужой урок", video_url="https://youtube.com"
        )
    ...
    def test_lesson_crud_permissions(self):
        ...
        response = client.post(
            "/api/courses/lessons/",
            {"title": "Новый урок", "course": self.course1.id, "video_url": "https://youtube.com"},
        )
        ...
        response = client.put(
            f"/api/courses/lessons/{self.lesson2.id}/",
            {"title": "Попытка взлома", "course": self.course2.id, "video_url": "https://youtube.com"},
        )
    ...
    def test_course_crud_permissions(self):
        ...
        response = client.post("/api/courses/courses/", {"title": "Новый курс от user1", "description": "Описание"})
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)


        # --- ТЕСТ 2: Пользователь НЕ может УДАЛИТЬ чужой курс ---
        response = client.delete(f"/api/courses/courses/{self.course2.id}/")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
